...
def main():
    ...
    parser.add_argument('--ro_neg', type=float, default=0.2, help='bag ratio for negtive')
    ...
    parser.add_argument('--gpuid', type=str, default="0", help='Gpu type, 0: single gpu, 1: 2 gpus')
    ...
    parser.add_argument('--threshold', type=float, default=0.3, help='threshold of binarizing the pseudo label')

    parser.add_argument('--temperature', type=float, default=0.07, help='temperature')
    parser.add_argument('--model_save_root', type=str, default='model_save', help='model save root')
    parser.add_argument('--lr', type=float, default=0.01, help='learning rate of training the feature extractor')


    parser.add_argument('--mask_uncertain_neg', type=str2bool, default=True, help='whether to mask the instance with negative pseudo label whose bag label is postitive')
    
    parser.add_argument('--negfilter', type=str2bool, default=True, help='whether to filter the negative patches for fine-tuning')
    parser.add_argument('--neg_ratio', type=float, default=0.2, help='the ratio to filter the negative patches')
    parser.add_argument('--validate_on_all_instance', type=str2bool, default=True, help='whether to validate on all instances or not')

    parser.add_argument('--augment_transform', type=int, default=0, help='which type of data augmentation')
    parser.add_argument('--MIL_every_n_epochs', type=int, default=1, help='conduct MIL training every number of epoch')
    ...
    # ranking loss
    parser.add_argument('--ranking_loss', type=str2bool, default=True, help='whether to use ranking loss or not')
    parser.add_argument('--rank_k', type=int, default=10, help='top k for ranking loss')
    parser.add_argument('--loss_weight_ranking', type=float, default=0.1, help='loss weight for ranking loss')
    ...
    gpu_ids = [int(x) for x in args.gpuid.split(',')]
    config['n_gpu'] = len(gpu_ids)
    config['gpu_ids'] = '(' + ','.join([str(id) for id in gpu_ids]) + ')'
    ...
    # seperate the positive and negative patches based on the threshold
    train_dataset_pos = InssepSPLDataset(args.root_dir, 'dataset/CAMELYON16/large_train_bags.txt', transform, pseudo_label=args.pseudo_label_path, threshold=args.threshold, witness_rate=args.witness_rate, posbag=True, mask_uncertain_neg=args.mask_uncertain_neg, ratio=args.ro, use_ema=args.use_ema, labelroot=args.labelroot, negfilter=args.negfilter, neg_ratio=args.neg_ratio)
    train_dataset_neg = InssepSPLDataset(args.root_dir, 'dataset/CAMELYON16/large_train_bags.txt', transform, pseudo_label=args.pseudo_label_path, threshold=args.threshold, witness_rate=args.witness_rate, posbag=False, mask_uncertain_neg=args.mask_uncertain_neg, ratio=0, use_ema=args.use_ema, labelroot=args.labelroot, negfilter=args.negfilter, neg_ratio=args.neg_ratio)


    train_dataset_pos_full = InssepDataset(args.root_dir, 'dataset/CAMELYON16/large_train_bags.txt', transform, pseudo_label=args.pseudo_label_path, threshold=args.threshold, witness_rate=args.witness_rate, posbag=True, mask_uncertain_neg=args.mask_uncertain_neg,labelroot=args.labelroot)
    train_dataset_neg_clean = InssepSPLDataset(args.root_dir, 'dataset/CAMELYON16/large_train_bags.txt', transform, pseudo_label=args.pseudo_label_path, threshold=args.threshold, witness_rate=args.witness_rate, posbag=False, mask_uncertain_neg=args.mask_uncertain_neg, ratio=0, use_ema=args.use_ema, labelroot=args.labelroot, negfilter=args.negfilter, neg_ratio=args.neg_ratio)
    
    print("positive ratio before filtering: ", len(train_dataset_pos.tiles))

    val_dataset = InsDataset(args.root_dir, 'dataset/CAMELYON16/val_bags.txt', transform, pseudo_label=args.pseudo_label_path, threshold=args.threshold, witness_rate=args.witness_rate, labelroot=args.labelroot)
    if not args.validate_on_all_instance:
        val_dataset.tiles = train_dataset_pos.tiles + train_dataset_neg_clean.tiles
    test_dataset = InsDataset(args.root_dir, 'dataset/CAMELYON16/test_bags.txt', transform, pseudo_label=None, threshold=args.threshold, witness_rate=args.witness_rate, labelroot=args.labelroot)
