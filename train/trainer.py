...
from sklearn.metrics import roc_curve, roc_auc_score, precision_recall_fscore_support, precision_recall_curve, confusion_matrix, auc
...
def get_features(net, dataset):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=1024, shuffle=False, num_workers=16)
    with torch.no_grad():
...
class SimCLR(object):
    ...
    def _validate(self, model, valid_loader):
        ...
        with torch.no_grad():
            ...
            valid_loss = 0.0
            counter = 0
            for xis, xjs, bag_label, label, gt_label, slide_name, patch_name in tqdm(valid_loader, desc="Validation", ncols=100):
    ...
    def train_MIL(self, root, epoch_count, args):
        ...
        labels = pd.read_csv(self.args.labelroot + '/testing/bag_reference.csv', header=None)
        ...
        for i, b in enumerate(test_dataset):
            ...
            #                      , b[0].numpy(), b[2], b[3]])
            test_list.append([int(labels.iloc[i][1] == 1)
                                 , b[0].numpy(), b[2], b[3]])
        ...
        if args.ranking_loss == True:
            print("Using Ranking Loss")
        ...
        for epoch in range(0, args.num_epoch_mil):
            ...
            if auc_value > optimal_ac:
                ...
                cm = confusion_matrix(bag_labels, np.round(bag_predictions))
                print("Confusion Matrix when AUC is optimal:", cm)
        ...
        print('\n Optimal_MIL_auc_test: %.4f ' % (optimal_ac))
        print('\n Optimal_test_Accuracy_when_AUC_optimal: %.4f ' % (optimal_accuracy))
        print('\n Optimal_test_Fscore_when_AUC_optimal: %.4f ' % (optimal_fscore))
        
        print('\n Optimal_MIL_auc_val: %.4f ' % (optimal_ac_val))

        self.experiment.log_metric('Optimal_MIL_auc_test', optimal_ac, epoch=epoch_count)
        self.experiment.log_metric('Optimal_test_Accuracy_when_AUC_optimal', optimal_accuracy, epoch=epoch_count)
        self.experiment.log_metric('Optimal_test_Fscore_when_AUC_optimal', optimal_fscore, epoch=epoch_count)
        ...
        auc_value, accuracy, precision_at_max_f1, recall_at_max_f1, auprc, max_f1_threshold, max_f1_score = self.eval_pseudo_label(root, ins_predictions, train=False)
        self.experiment.log_metric('Test_ins_auc', auc_value, epoch=epoch_count)
        self.experiment.log_metric('Test_ins_accuracy', accuracy, epoch=epoch_count)
        self.experiment.log_metric('Test_ins_precision_at_max_f1', precision_at_max_f1, epoch=epoch_count)
        self.experiment.log_metric('Test_ins_recall_at_max_f1', recall_at_max_f1, epoch=epoch_count)
        self.experiment.log_metric('Test_ins_auprc', auprc, epoch=epoch_count)
        self.experiment.log_metric('Test_ins_max_f1_threshold', max_f1_threshold, epoch=epoch_count)
        self.experiment.log_metric('Test_ins_max_f1_score', max_f1_score, epoch=epoch_count)
        
        print('\n Test_ins_auc: %.4f ' % (auc_value))
        print('\n Test_ins_accuracy: %.4f ' % (accuracy))
        print('\n Test_ins_precision_at_max_f1: %.4f ' % (precision_at_max_f1))
        print('\n Test_ins_recall_at_max_f1: %.4f ' % (recall_at_max_f1))
        print('\n Test_ins_auprc: %.4f ' % (auprc))
        print('\n Test_ins_max_f1_threshold: %.4f ' % (max_f1_threshold))
        print('\n Test_ins_max_f1_score: %.4f ' % (max_f1_score))
        ...
        auc_value, accuracy, precision_at_max_f1, recall_at_max_f1, auprc, max_f1_threshold, max_f1_score = self.eval_pseudo_label(root, ins_predictions)
        self.experiment.log_metric('Pseudo_ins_auc', auc_value, epoch=epoch_count)
        self.experiment.log_metric('Pseudo_ins_accuracy', accuracy, epoch=epoch_count)
        self.experiment.log_metric('Pseudo_ins_precision_at_max_f1', precision_at_max_f1, epoch=epoch_count)
        self.experiment.log_metric('Pseudo_ins_recall_at_max_f1', recall_at_max_f1, epoch=epoch_count)
        self.experiment.log_metric('Pseudo_ins_auprc', auprc, epoch=epoch_count)
        self.experiment.log_metric('Pseudo_ins_max_f1_threshold', max_f1_threshold, epoch=epoch_count)
        self.experiment.log_metric('Pseudo_ins_max_f1_score', max_f1_score, epoch=epoch_count)
    ...
    def eval_pseudo_label_malbag(self, root, ins_predictions, train=True):
        ...
        for name in train_name:
            if "tumor" in name[0]:
                x, y, z = map(int, name[1].split('_'))
                try:
                    y_prob_list.append(pred_test[name[0]][name[1]])
                    train_y_list.append(train_gt[(name[0]+'/', x, y)])
                    # y_prob_list.append(pred_test[name[0]][name[1]])
                except KeyError: # assuming that the exception is a KeyError, adjust if needed
                    continue
                # train_y_list.append(train_gt[(name[0]+'/', x, y)])
                # y_prob_list.append(pred_test[name[0]][name[1]])
    ...
    def eval_pseudo_label(self, root, ins_predictions, train=True):
        ...
        def extract_values_from_name_gt(name):
            x, y, z = map(int, name[1].split('_'))
            return train_gt[(name[0]+'/', x, y)]
        ...
        train_y = np.array([extract_values_from_name_gt(name) for name in train_name])
        y_prob = np.array([pred_test[name[0]][name[1]] for name in train_name])
        
        accuracy, auc_value, _, _, _ = five_scores(train_y, y_prob)
        precision, recall, thresholds = precision_recall_curve(train_y, y_prob)
        # calculate the auprc
        ...
        return auc_value, accuracy, precision_at_max_f1, recall_at_max_f1, auprc, max_f1_threshold, max_f1_score
    ...
    def extract_feature(self, state_dict_path, args):
        ...
        # root_dir = '/single'
        # root_dir = '/scr/wentao/Project/mil/WSI/CAMELYON16/single_b10_t30'
        root_dir = '/data2/wentao/Project/MICCAI2024/mil/WSI/CAMELYON16/single_b10_t30'

        train_dataset = SSLDataset(root_dir, 'dataset/CAMELYON16/train_bags.txt',
                                   transform=augmentation['val'], witness_rate=args.witness_rate, labelroot=args.labelroot)
        val_dataset = SSLDataset(root_dir, 'dataset/CAMELYON16/val_bags.txt',
                                 transform=augmentation['val'], witness_rate=args.witness_rate,  labelroot=args.labelroot)
        test_dataset = SSLDataset(root_dir, 'dataset/CAMELYON16/test_bags.txt',
                                  transform=augmentation['val'], witness_rate=args.witness_rate,  labelroot=args.labelroot)
    ...
    def train(self):
        ...
        if self.config['n_gpu'] > 1:
            # device_ids = eval(self.config['gpu_ids'])
        ...
        if self.args.init_MIL_training:
            ...
            # self.extract_feature(os.path.join(model_checkpoints_folder, 'epoch' + 'init' + 'model.pth'), self.args)
        ...
        for epoch_counter in range(self.config['epochs']):
            ...
            for xis, xjs, bag_label, label, gt_label, slide_name, patch_name in tqdm(self.train_loader_neg_clean, desc="Training", ncols=100):
                try:
            ...
            print("saving model epoch {}".format(epoch_counter))
            ...
            if epoch_counter >= self.args.warmup:
                ...
                print("self paced learning epoch {}".format(epoch_counter))
            ...
            if epoch_counter >= 1 and epoch_counter % self.config['MIL_every_n_epochs'] == 0:
                ...
                print("updating pseudo label epoch {}".format(epoch_counter))
                ...
                print("extract feature for MIL training epoch {}".format(epoch_counter))

                self.extract_feature(os.path.join(model_checkpoints_folder, 'epoch' + str(epoch_counter) + 'model.pth'),
                                     self.args)

                # train MIL and get the instance pseudo label
                print("continue MIL training epoch {}".format(epoch_counter))
            ...
            if epoch_counter % self.config['eval_every_n_epochs'] == 0:
                ...
                print("validate the model epoch {}".format(epoch_counter))
                ...
                if valid_loss < best_valid_loss:
                    ...
                    print('better validate model saved at epoch {}'.format(epoch_counter))
...
def load_pretrained_simCLR(net, model_dir):
    ...
    # if not multigpu:
    #     model_state_dict = {k.replace("module.resnet.", "resnet."): v for k, v in
    #                         state_dict.items() if "resnet" in k}
    # else:
    #     model_state_dict = {k.replace("module.resnet.", "module.resnet."): v for k, v in
    #                         state_dict.items() if "resnet" in k}
